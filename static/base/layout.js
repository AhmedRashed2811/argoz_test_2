  ...
  function renderPanel() {
    ...
    if (catContainer) {
      ...
      catContainer.querySelectorAll('.notif-cat-tab').forEach(tab => {
        ...
        const count = NOTIFS.filter(item => matchCategory(item, cat) && !item.is_read).length;
        let baseText = cat === 'leads' ? 'Leads' :
                       cat === 'reminders' ? 'Reminders' :
                       cat === 'meetings' ? 'Meetings' :
                       cat === 'followups' ? 'Follow-ups' :
                       cat === 'campaigns' ? 'Campaigns' : '';
        if (count > 0) {
          tab.innerHTML = `${baseText} <span class="notif-cat-badge">${count}</span>`;
        } else {
          tab.innerHTML = baseText;
        }
    ...
    notifBody.innerHTML = `<div class="notif-group-label">${activeCat.toUpperCase()}</div>` + filtered.map((item) => {
      ...
      const markReadBtn = !item.is_read
        ? `<button type="button" class="notif-mark-read" data-id="${item.id}" title="Mark as Read"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></button>`
        : '';
      const deleteBtn = `<button type="button" class="notif-delete-btn" data-id="${item.id}" title="Delete Notification"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2 2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></button>`;
